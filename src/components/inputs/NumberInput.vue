...
export default class NumberInput extends Mixins(BaseMixin) {
    ...
    // when true, clicking/tabbing away submits the typed value instead of
    // discarding it -- opt-in so existing call sites that rely on
    // Enter-to-commit (the historical default everywhere else in Mainsail)
    // keep their current behaviour unchanged.
    @Prop({ type: Boolean, required: false, default: false }) declare readonly commitOnBlur: boolean
    ...
    handleBlur(): void {
        if (this.commitOnBlur) this.submit()
        else this.value = this.target.toString()
    }
